...
public:
  ...
  Iterator find(const Key_type& k) const{
    for(Iterator i = begin(); i != end(); i++){
      if(!Key_compare{}(i->first, k)&&!Key_compare{}(k, i->first)) return i;
    }
    return end();
  };
  ...
  std::pair<Iterator, bool> insert(const Pair_type &val){
    // If element in the tree
    if(map_tree.find(val)!=end()){
      return {map_tree.find(val), false};
    }else{
      // If element not in the tree
      return {map_tree.insert(val), true};}
  };
