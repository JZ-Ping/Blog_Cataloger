...
// ---------- DO NOT CHANGE ANYTHING IN THIS FILE ABOVE THIS LINE ----------
  ...
  static Node * find_impl(Node *node, const T &query, Compare less) {
    ...
    if(less(query, node->datum)){
      nodeptr = find_impl(node->left, query, less);
    } else if(less(node->datum,query)){
      nodeptr = find_impl(node->right, query, less);
    } else{
      nodeptr = node;
    }
    return nodeptr;
  ...
  static Node * insert_impl(Node *node, const T &item, Compare less) {
    ...
    if(less(item,node->datum)){
      ...
      if(node->left!=nullptr){
        newptr = insert_impl(node->left, item, less);
      } else {
      ...
      if(node->right!=nullptr){
        newptr = insert_impl(node->right, item, less);
      } else {
        newptr = new Node;
        node->right = newptr;
        newptr->datum = item;
  ...
  static Node * min_element_impl(Node *node) {
    ...
    Node* minptr;
    if(node->left!=nullptr){
      minptr = min_element_impl(node->left);
    } else {
      minptr = node;
    }
    return minptr;
  ...
  static Node * max_element_impl(Node *node) {
    ...
    Node* minptr;
    if(node->right!=nullptr){
      minptr = max_element_impl(node->right);
    } else {
      minptr = node;
    }
    return minptr;
  ...
  static bool check_sorting_invariant_impl(const Node *node, Compare less) {
    ...
    if(node->left){
      left_check = less(node->left->datum, node->datum) && 
          check_sorting_invariant_impl(node->left, less);
    }
    if(node->right){
      right_check = less(node->datum,node->right->datum) &&
